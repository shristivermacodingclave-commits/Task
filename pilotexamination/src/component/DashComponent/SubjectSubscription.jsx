...
export default function SubjectSubscription() {
  ...
  const fetchPlanData = async () => {
    try {
      ...
      // ⭐ Save subject data from API
      setSubject(res.data.subject || {});

      // ⭐ Save topics
      setTopics(res.data.topics || []);

      // ⭐ Convert subscription plans
      if (res.data.plans) {
        const apiPlans = res.data.plans.map((p) => {
          const discountPercent = p.plan_price
            ? Math.round(((p.plan_price - p.selling_price) / p.plan_price) * 100)
      ...
      <div className="row justify-content-center align-items-start g-4">
        ...
        <div className="col-md-4">
          <div className="card border-0 shadow-sm rounded-4 h-100">
            ...
            {/* ⭐ DYNAMIC SUBJECT IMAGE */}
            <img
              src={
                subject.subject_image?.startsWith("http")
                  ? subject.subject_image
                  : `https://development.pilotexaminations.com/${subject.subject_image}`
              }
            ...
            <div className="card-body">
              <h3 className="card-title my-3">{subject.subject_name}</h3>

              {/* ⭐ DYNAMIC TOPICS */}
              <ul className="mb-0 ps-3 small fs-6">
                {topics.length === 0 ? (
                  <li><Loader message="Loading topics..." compact /></li>
                ) : (
                  topics.slice(0, 4).map((t) => (
