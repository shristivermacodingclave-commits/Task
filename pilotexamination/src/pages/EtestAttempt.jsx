...
export default function EtestAttempt() {
  ...
  const testMeta = location.state || {};
  const type = testMeta.type || "etest";   // etest | practice-test | mock-test
  ...
  const mockAttemptId =
    searchParams.get("mock_attempt_id") ||
    testMeta.mockAttemptId ||
    localStorage.getItem("mock_attempt_id");

  // ================================
  // CORRECT QUESTION LOADING LOGIC
  // ================================

  let storedQuestions = [];

  if (testMeta.questions) {
    storedQuestions = testMeta.questions; // came directly from Start page
  } else {
    if (type === "practice-test") {
      storedQuestions = JSON.parse(localStorage.getItem("practice_questions") || "[]");
    } else if (type === "mock-test") {
      storedQuestions = JSON.parse(localStorage.getItem("mock_questions") || "[]");
    } else {
      storedQuestions = JSON.parse(localStorage.getItem("questions") || "[]"); // e-test
    }
  }
  ...
  // ======================================================
  // AUTO LOAD SAVED ANSWERS FROM SERVER
  // ======================================================
  useEffect(() => {
    (async function () {
      setLoading(true);
      ...
      try {
        ...
        // mock test ----------------------------------------------
          ...
          res = await axios.post(
            "https://development.pilotexaminations.com/api/mock/attempt",
            { test_id: mockAttemptId }
          );
        }
        ...
        console.log("Error loading attempt", error);
      }

      setLoading(false);
    })();
  }, [attemptId, practiceTestId, mockAttemptId, type]);
          ...
          <div className="etest-modal">
            ...
            <div className="etest-modal-body">
              ...
              <div className="summary-row">
                <div className="summary-item">
                  <span className="summary-box attempted-box"></span>
                  Attempted ({answers.filter((a) => a.confirmed).length})
                </div>
